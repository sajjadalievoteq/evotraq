...
import 'package:traqtrace_app/data/models/operations/return_receiving/return_receiving_page_model.dart';
...
class ReturnReceivingOperationService {
  ...
  Future<ReturnReceivingPage> getReturnReceivingOperationsPage({
    int page = 0,
    int size = 20,
  }) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final operations = (responseData['operations'] as List? ?? const [])
            .map((op) => ReturnReceivingResponse.fromJson(op as Map<String, dynamic>))
            .toList();
  ...
  void _logApiException(
    ...
    debugPrint(
      '[ReturnReceivingOperationService] ApiException '
      'status=${exception.statusCode} message=${exception.message}',
    );
    if (exception.responseBody != null && exception.responseBody!.isNotEmpty) {
      debugPrint(
        '[ReturnReceivingOperationService] responseBody: ${exception.responseBody}',
      );
    }
    if (stackTrace != null) {
      debugPrint('[ReturnReceivingOperationService] $stackTrace');
    }
  }
}
