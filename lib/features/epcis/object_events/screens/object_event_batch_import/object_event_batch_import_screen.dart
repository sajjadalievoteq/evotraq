...
import 'package:traqtrace_app/features/epcis/object_events/screens/object_event_batch_import/widgets/object_event_batch_import_csv_tab.dart';
import 'package:traqtrace_app/features/epcis/object_events/screens/object_event_batch_import/widgets/object_event_batch_import_manual_tab.dart';
import 'package:traqtrace_app/core/widgets/traq_icon.dart';
import 'package:traqtrace_app/core/config/app_assets.dart';

class ObjectEventBatchImportScreen extends StatefulWidget {
  const ObjectEventBatchImportScreen({Key? key, this.embedded = false})
    : super(key: key);

  final bool embedded;

  @override
  State<ObjectEventBatchImportScreen> createState() =>
      _ObjectEventBatchImportScreenState();
}

class _ObjectEventBatchImportScreenState
    extends State<ObjectEventBatchImportScreen>
    ...
    if (_formKey.currentState!.validate()) {
      ...
      for (String epc in _epcList) {
        final event = ObjectEvent(
          eventId:
              'urn:epcglobal:cbv:epcis:event:${DateTime.now().millisecondsSinceEpoch}-${_pendingEvents.length}',
          eventTime: now,
          recordTime: now,
          eventTimeZone: '+00:00',
  ...
  List<ObjectEvent> _createEventsFromCsv() {
    ...
    for (Map<String, dynamic> row in _csvData) {
      try {
        ...
        String? businessStep =
            row['BusinessStep'] ?? row['businessStep'] ?? row['business_step'];
        ...
        String? readPoint =
            row['ReadPoint'] ?? row['readPoint'] ?? row['read_point'];
        ...
        final event = ObjectEvent(
          eventId:
              'urn:epcglobal:cbv:epcis:event:${now.millisecondsSinceEpoch}-${events.length}',
          eventTime: now,
          recordTime: now,
          eventTimeZone: '+00:00',
          epcisVersion: EPCISVersion.v2_0,
          ...
          businessLocation:
              businessLocation != null && businessLocation.isNotEmpty
  ...
  Widget build(BuildContext context) {
    final hasPending =
        ...
        (_tabController.index == 1 && _csvData.isNotEmpty);
    final tabs = TabBar(
