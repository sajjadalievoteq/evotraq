...
import 'package:traqtrace_app/features/epcis/presentation/object_events/presentation/form/utilities/cbv_vocabulary_constants.dart';
...
class _CbvBizStepDispositionPickerState
  ...
  void _applyActionDefaults(CbvVocabularyState state) {
    final allowed = _bizStepsFor(widget.action, state.bizSteps);
    ...
    if (currentCode == null || !allowed.any((i) => i.code == currentCode)) {
      final defaultCode =
          CbvVocabularyConstants.actionDefaultBizStepCode[widget.action];
  ...
  List<CbvVocabularyItem> _bizStepsFor(
      String? action, List<CbvVocabularyItem> all) {
    final codes = CbvVocabularyConstants.actionBizStepCodes[action];
    if (codes == null) return all;
    // System items follow the per-action allowlist from the CBV spec.
  ...
  List<CbvVocabularyItem> _dispositionsFor(
    ...
    final codes = CbvVocabularyConstants.bizStepDispositionCodes[bizCode];
    if (codes == null || codes.isEmpty) return all;
    return all.where((d) => codes.contains(d.code)).toList();
  }
  ...
  Widget _buildBizStepDropdown(CbvVocabularyState state) {
    ...
    final bizSteps = _bizStepsFor(widget.action, state.bizSteps);
    if (bizSteps.isEmpty) {
      return const Text(
