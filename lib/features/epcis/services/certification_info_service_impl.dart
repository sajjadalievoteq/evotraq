...
import 'package:traqtrace_app/features/epcis/services/certification_info_service.dart';

/// Implementation of the CertificationInfoService interface
class CertificationInfoServiceImpl implements CertificationInfoService {
  ...
  Future<List<CertificationInfo>> getAllCertifications() async {
    ...
    if (response.statusCode == 200) {
      ...
      return certificationList.map((json) => CertificationInfo.fromJson(json)).toList();
    } else {
  ...
  @override
  Future<Map<String, dynamic>> getAllCertificationsPaginated(int page, int size) async {
    final headers = await _getHeaders();
    final response = await _httpClient.get(
    ...
    if (response.statusCode == 200) {
      ...
      responseData['content'] = content.map((json) => 
        CertificationInfo.fromJson(json)).toList();
        
      return responseData;
    } else {
      throw Exception('Failed to get paginated certifications: ${response.statusCode}');
    }
  }
  ...
  @override
  Future<CertificationInfo> createCertification(CertificationInfo certificationInfo) async {
    final headers = await _getHeaders();
  ...
  @override
  Future<CertificationInfo> updateCertification(String id, CertificationInfo certificationInfo) async {
    final headers = await _getHeaders();
  ...
  @override
  Future<List<CertificationInfo>> getCertificationsByEventId(String eventId) async {
    final headers = await _getHeaders();
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Failed to get certifications by event: ${response.statusCode}');
    }
  }
  ...
  Future<List<CertificationInfo>> getCertificationsByType(String type) async {
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Failed to get certifications by type: ${response.statusCode}');
    }
  }
  
  @override
  Future<List<CertificationInfo>> getCertificationsByAgency(String agency) async {
    final headers = await _getHeaders();
    final response = await _httpClient.get(
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Failed to get certifications by agency: ${response.statusCode}');
    }
  }
