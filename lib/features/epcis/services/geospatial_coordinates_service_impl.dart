...
import 'package:traqtrace_app/features/epcis/services/geospatial_coordinates_service.dart';
...
class GeospatialCoordinatesServiceImpl implements GeospatialCoordinatesService {
  ...
  /// Constructor
  GeospatialCoordinatesServiceImpl({
  ...
  Future<List<GeospatialCoordinates>> getAllGeospatialCoordinates() async {
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Failed to get geospatial coordinates: ${response.statusCode}');
    }
  }
  
  @override
  Future<Map<String, dynamic>> getAllGeospatialCoordinatesPaginated(int page, int size) async {
    final headers = await _getHeaders();
    final response = await _httpClient.get(
    ...
    if (response.statusCode == 200) {
      ...
      responseData['content'] = content.map((json) => 
        GeospatialCoordinates.fromJson(json)).toList();
        
      return responseData;
    } else {
      throw Exception('Failed to get paginated geospatial coordinates: ${response.statusCode}');
    }
  }
  ...
  @override
  Future<GeospatialCoordinates> createGeospatialCoordinates(GeospatialCoordinates coordinates) async {
    final headers = await _getHeaders();
    ...
    if (response.statusCode == 201) {
      ...
      throw Exception('Failed to create geospatial coordinates: ${response.statusCode}');
    }
  }
  ...
  Future<GeospatialCoordinates> updateGeospatialCoordinates(String id, GeospatialCoordinates coordinates) async {
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Failed to update geospatial coordinates: ${response.statusCode}');
    }
  }
  ...
  @override
  Future<GeospatialCoordinates?> getGeospatialCoordinatesByGLN(String glnCode) async {
    final headers = await _getHeaders();
    final response = await _httpClient.get(
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Failed to get coordinates by GLN: ${response.statusCode}');
    }
  }
  
  @override
  Future<GLN> addCoordinatesToLocation(String glnCode, GeospatialCoordinates coordinates) async {
    final headers = await _getHeaders();
    final response = await _httpClient.post(
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Failed to add coordinates to location: ${response.statusCode}');
    }
  }
  
  @override
  Future<List<GLN>> findLocationsByProximity(double latitude, double longitude, double radiusKm) async {
    final headers = await _getHeaders();
    final response = await _httpClient.get(
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Failed to find nearby locations: ${response.statusCode}');
    }
  }
}
