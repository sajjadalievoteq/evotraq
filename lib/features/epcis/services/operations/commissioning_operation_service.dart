...
/// Service for handling bulk commissioning operations.
/// 
/// This service creates SGTINs and their corresponding commissioning ObjectEvents
/// in a single batch operation.
abstract class CommissioningOperationService {
  /// Creates a bulk commissioning operation
  Future<CommissioningResponse> createCommissioningOperation(
      CommissioningRequest request);

  /// Gets all commissioning operations (from aggregation events)
  Future<List<CommissioningResponse>> getCommissioningOperations();

  /// Gets a specific commissioning operation by ID
  Future<CommissioningResponse?> getCommissioningOperation(String operationId);
}

class CommissioningOperationServiceImpl implements CommissioningOperationService {
  ...
  Future<CommissioningResponse> createCommissioningOperation(
    ...
    debugPrint('CommissioningService: Starting bulk commissioning for ${request.serialNumbers.length} items');
    ...
    try {
      ...
      for (int i = 0; i < request.serialNumbers.length; i++) {
        ...
        try {
          ...
          debugPrint('CommissioningService: Error commissioning $serialNumber: $e');
        }
      }
      ...
      return CommissioningResponse(
        ...
        messages: messages.isEmpty ? ['Successfully commissioned $successCount items'] : messages,
      ...
      debugPrint('CommissioningService: Critical error during commissioning: $e');
      return CommissioningResponse(
        commissioningOperationId: operationId,
        commissioningReference: request.commissioningReference,
        status: CommissioningStatus.failed,
  ...
  @override
  Future<CommissioningResponse?> getCommissioningOperation(String operationId) async {
    try {
      final headers = await _getHeaders();
      final response = await _client.get(
  ...
  CommissioningResponse _parseObjectEventToCommissioningResponse(Map<String, dynamic> event) {
