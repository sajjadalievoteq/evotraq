...
class EpcisImportValidationResult {
  const EpcisImportValidationResult({
...
abstract final class EpcisImportValidator {
  static EpcisImportValidationResult validate(
    ...
    final contextOk = context == EpcisImportTemplate.contextUri ||
        (context is List &&
            context.any((c) => c == EpcisImportTemplate.contextUri));
    if (!contextOk) {
      formatIssues.add(const EpcisImportIssue(
        gate: EpcisImportGate.format,
        path: r'$["@context"]',
        reason:
            'Input does not match the EPCIS import template — download the template.',
    ...
    if (schemaVersion != '2.0') {
      schemaIssues.add(EpcisImportIssue(
        gate: EpcisImportGate.schema,
        path: r'$.schemaVersion',
        reason: 'schemaVersion must be "2.0" (found: ${schemaVersion ?? 'null'})',
      ));
    }
    if (doc['creationDate'] is! String ||
        (doc['creationDate'] as String).trim().isEmpty) {
      schemaIssues.add(const EpcisImportIssue(
        gate: EpcisImportGate.schema,
        path: r'$.creationDate',
    ...
    for (final placeholder in EpcisImportTemplate.placeholders) {
      if (serialized.contains(placeholder)) {
        contentIssues.add(EpcisImportIssue(
  ...
  static void _validateEventSchema(
    ...
    _checkUnknownKeys(event.keys.cast<String>(), allowed, path, issues);
  ...
  static void _validateEventContent(
    Map<String, dynamic> event,
    String path,
    List<EpcisImportIssue> issues,
    ...
    void checkEpc(String? epc, String epcPath) {
      ...
      if (!Gs1CanonicalIdentifier.isValid(trimmed) &&
          Gs1CanonicalIdentifier.classify(trimmed) == Gs1CanonicalKind.unknown) {
        issues.add(EpcisImportIssue(
          gate: EpcisImportGate.content,
          path: epcPath,
    ...
    if (ilmd is Map && ilmd['cbvmda:itemExpirationDate'] != null) {
      ...
      if (!RegExp(r'^\d{4}-\d{2}-\d{2}$').hasMatch(exp)) {
        ...
        final err = Gs1DateUtils.validateYymmdd(yymmdd, label: 'itemExpirationDate');
        if (err != null) {
          issues.add(EpcisImportIssue(
            gate: EpcisImportGate.content,
            path: '$path.ilmd["cbvmda:itemExpirationDate"]',
  ...
  static void _validateIsoInstant(
    String? value,
    String path,
    List<EpcisImportIssue> issues,
  ) {
