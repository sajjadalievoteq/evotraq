...
import 'package:traqtrace_app/features/operations/decommissioning/screens/decommissioning_operation_list/utils/decommissioning_operation_list_filter.dart';
import 'package:traqtrace_app/features/operations/decommissioning/screens/decommissioning_operation_list/widgets/decommissioning_advanced_filters_panel.dart';
import 'package:traqtrace_app/features/operations/decommissioning/screens/decommissioning_operation_list/widgets/decommissioning_operation_list_results.dart';
import 'package:traqtrace_app/features/operations/decommissioning/screens/decommissioning_operation_list/widgets/decommissioning_quick_filter_dialog.dart';
import 'package:traqtrace_app/features/operations/decommissioning/screens/decommissioning_operation_list/widgets/decommissioning_record_info_section.dart';
...
/// Screen to list all Decommissioning operations with search capabilities.
class DecommissioningOperationListScreen extends StatefulWidget {
  const DecommissioningOperationListScreen({
    super.key,
    this.embedded = false,
  ...
  @override
  State<DecommissioningOperationListScreen> createState() =>
      _DecommissioningOperationListScreenState();
}

class _DecommissioningOperationListScreenState extends State<DecommissioningOperationListScreen> {
  final TextEditingController _searchController = TextEditingController();
  final _scrollController = ScrollController();

  List<DecommissioningResponse> _operations = [];
  int _totalRecords = 0;
  List<DecommissioningResponse> _filteredOperations = [];
  ...
  void _showFilterDialog() {
    DecommissioningQuickFilterDialog.open(
      context,
  ...
  void _showAdvancedFiltersDialog() {
    showDialog<void>(
      ...
      builder: (dialogContext) => AlertDialog(
        title: const Text(DecommissioningUiConstants.advancedFiltersTitle),
        content: ConstrainedBox(
          constraints: const BoxConstraints(maxWidth: 480),
          child: StatefulBuilder(
            builder: (context, setLocalState) {
              return DecommissioningAdvancedFiltersPanel(
                sortBy: _sortBy,
  ...
  String _sortFieldDisplayLabel() {
    return DecommissioningUiConstants.sortFieldLabels[_sortBy] ??
        DecommissioningUiConstants.sortFieldFallback;
  }
  ...
  Future<void> _loadOperations() async {
    ...
    try {
      ...
      final page = await service.getDecommissioningOperationsPage(page: 0);
      setState(() {
      ...
      setState(() {
        _errorMessage =
            'Could not load Decommissioning operations. Check your connection and tap Retry.';
      });
    } finally {
  ...
  Future<void> _loadMoreOperations() async {
    ...
    try {
      ...
      final page = await service.getDecommissioningOperationsPage(page: nextPage);
      setState(() {
  ...
  void _syncEmbeddedOperationIds() {
    ...
    final ids = _filteredOperations
        ...
        .whereType<String>()
        .toList();
    context.read<DecommissioningOperationsCubit>().updateOperationIds(
          ids,
          isEmpty: ids.isEmpty,
        );
  }
  ...
  void _filterOperations() {
    setState(() {
      _filteredOperations = DecommissioningOperationListFilter.apply(
  ...
  Widget build(BuildContext context) {
    final body = Gs1MasterListBody(
          ...
          Padding(
            ...
            child: Column(
              children: [
                ListenableBuilder(
                  ...
                  builder: (context, _) {
                    return Gs1ListSearchBar(
                      hintText: DecommissioningUiConstants.listSearchHint,
                ...
                DecommissioningRecordInfoSection(
                ...
                Gs1ListSortingControls(
                  label: DecommissioningUiConstants.sortByLine(
                    _sortFieldDisplayLabel(),
                    _sortDir == 'asc'
                        ? DecommissioningUiConstants.sortAscendingLabel
                        : DecommissioningUiConstants.sortDescendingLabel,
                  ),
                  sortOrder: _sortDir,
      ...
      results: DecommissioningOperationListResults(
    ...
    return Scaffold(
      appBar: TraqAppBar(
        context,
        title: const Text('Decommissioning Operation'),
      ),
      drawer: const AppDrawer(),
      floatingActionButton: widget.onEmbeddedCreate != null
          ? null
          : FloatingActionButton.extended(
              onPressed: () => context.go(Constants.opDecommissioningCreateRoute),
