...
/// User-facing error text for the create-Decommissioning flow.
abstract final class DecommissioningSubmitErrorMessage {
  ...
  static String emptyEpcList() =>
      'No valid items to decommission. Scan at least one SGTIN serial.';

  static String fromResponse(DecommissioningResponse response) {
    ...
    return switch (response.status) {
      DecommissioningStatus.validationError =>
        'Decommissioning validation failed. Check location GLN, disposition, and scanned items.',
      DecommissioningStatus.failed =>
        'Decommissioning could not be completed. Review scanned items and location details.',
      _ =>
        'The Decommissioning operation could not be completed. Check your inputs and try again.',
    };
  }
  ...
  static String unexpected(Object error) =>
      'An unexpected error occurred while submitting Decommissioning: $error';
}
