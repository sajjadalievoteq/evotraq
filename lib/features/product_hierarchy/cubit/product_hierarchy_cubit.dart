...
class ProductHierarchyCubit extends Cubit<ProductHierarchyState> {
  ...
  Future<void> loadRecentParents() async {
    ...
    try {
      ...
      if (isClosed) return;
      emit(
        state.copyWith(recentParents: const [], recentParentsLoading: false),
      );
    }
  }
  ...
  static List<PackingResponse> _distinctRecentParents(
    ...
    for (final op in operations) {
      final parent = normalizeProductHierarchyInput(op.parentContainerId ?? '');
  ...
  Future<void> openHierarchy(String rawInput) async {
    ...
    try {
      ...
      final rootHasParent = viewRootParent != null && viewRootParent.isNotEmpty;
  ...
  Future<void> climbToParent(String epc) async {
    ...
    try {
      ...
      emit(state.copyWith(isClimbing: false, hierarchyError: e.toString()));
    }
  }
