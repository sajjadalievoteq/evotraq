...
void main() {
  ...
  Map<String, dynamic> dashboard({
    ...
    'activeJobs': active,
    'completedJobs': 0,
    if (runQueued != null || runActive != null)
      'subscriptionRuns': {
        'queued': runQueued ?? 0,
        'active': runActive ?? 0,
  ...
  void stubRestBundle({int queued = 0, int active = 0}) {
    ...
    ).thenAnswer((_) async => List.generate(active, (i) => {'jobId': 'A$i'}));
    when(
      () => service.getQueuedJobs(limit: any(named: 'limit')),
    ).thenAnswer((_) async => List.generate(queued, (i) => {'jobId': 'Q$i'}));
    when(
      () => service.getJobHistory(limit: any(named: 'limit')),
  ...
  test(
    'incoming WS payload updates the snapshot via the shared builder',
    () async {
  ...
  test(
    'persisted subscription run counters override wrapper-job counters',
    () async {
      stubRestBundle(queued: 7, active: 2);
      when(() => service.getDashboard()).thenAnswer(
        (_) async => dashboard(
          queued: 7,
          active: 2,
          runQueued: 3,
      ...
      expect(cubit.state.snapshot!.queuedJobs, 3);
      expect(cubit.state.snapshot!.activeJobs, 1);
      expect(cubit.state.snapshot!.completedJobs, 12);
      expect(cubit.state.snapshot!.failedJobs, 4);
      await cubit.close();
    },
  );
